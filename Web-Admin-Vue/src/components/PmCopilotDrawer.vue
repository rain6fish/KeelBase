...
<template>
  <el-drawer v-model="open" :title="t('copilotTitle')" size="min(600px, 92vw)" @closed="onClosed">
    <!-- 对话区 -->
...
watch(open, (v) => {
  if (v) {
    // 每次打开重置对话（保留 conversationId=undefined → 新会话）
    items.value = []
...
async function send(raw: string) {
  ...
  // 首条消息自动注入当前项目上下文——问「这个项目的延期风险」AI 直接定位当前项目，不再反问
  const isFirst = items.value.length === 0
  const toBackend = isFirst ? `${projectContext()}。${message}` : message
