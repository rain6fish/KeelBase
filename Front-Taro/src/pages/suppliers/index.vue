<template>
  <view class="suppliers-page">
    <view class="suppliers-page__header">
      <text class="suppliers-page__title">供应商</text>
      <text class="suppliers-page__count">{{ items.length }} 条</text>
    </view>
    ...
    <view class="suppliers-page__input-bar">
      <input
        ...
        @confirm="handleAdd"
      />
      <button class="suppliers-page__add" size="mini" @click="handleAdd">添加</button>
    </view>

    <text v-if="store.isLoading" class="suppliers-page__hint">加载中…</text>
...
async function handleAdd() {
  ...
  if (!text) {
    Taro.showToast({ title: '请输入供应商内容', icon: 'none' })
    return
  }
  try {
