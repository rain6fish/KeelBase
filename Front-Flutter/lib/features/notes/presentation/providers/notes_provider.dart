...
/// 笔记状态管理（UX-1：缓存优先 + 乐观更新）。
class NotesProvider extends ChangeNotifier {
  final NotesRepository _repository;
  final AppCache _cache;

  static const _ns = 'notes';
  ...
  NotesProvider(this._repository, {AppCache? cache})
  ...
  Future<void> load() async {
    ...
    if (cached != null) {
      _items = cached.map(NoteModel.fromJson).toList();
      _fromCache = true;
      notifyListeners();
    }

    try {
      _items = await _repository.getNotes();
      _fromCache = false;
