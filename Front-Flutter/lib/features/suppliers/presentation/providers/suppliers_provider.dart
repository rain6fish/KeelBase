...
class SuppliersProvider extends ChangeNotifier {
  ...
  SuppliersProvider(this._repository, {AppCache? cache})
      : _cache = cache ?? AppCache.unavailable();

  List<SupplierModel> get items => _items;
  ...
  Future<void> load() async {
    ...
    if (cached != null) {
      _items = cached.map(SupplierModel.fromJson).toList();
      _fromCache = true;
      notifyListeners();
    }

    try {
      _items = await _repository.getSuppliers();
      _fromCache = false;
