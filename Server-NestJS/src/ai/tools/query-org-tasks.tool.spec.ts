...
describe('QueryOrgTasksTool (ORG-5)', () => {
  ...
  // 组织内审批待办统计已拆至 OrgDirectoryService（阶段 3 第十五刀）
  let orgDirectory: { getOrgApprovalTaskStats: jest.Mock };
  ...
  beforeEach(() => {
    ...
    tool = new QueryOrgTasksTool(orgService as any, orgDirectory as any);
  });
  ...
  it('非组织成员 → 错误', async () => {
    ...
    expect(orgDirectory.getOrgApprovalTaskStats).not.toHaveBeenCalled();
  });
  ...
  it('返回组织审批待办统计（组织边界数据）', async () => {
    ...
    expect(orgDirectory.getOrgApprovalTaskStats).toHaveBeenCalledWith(1);
  });
