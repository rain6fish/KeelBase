...
export class AiService {
  ...
  /** §internal.16 A-5 授权链：放行场景的「为什么允许」依据（tool 治理 checks + 角色 CASL 决策） */
  ...
  /** §internal.16 A-5 授权链图：按用户聚合完整授权链（角色→CASL 资源 + 工具策略 + 生效期） */
  ...
  /** §internal.16 A-1 Decision Evidence：analyze_* 确定性打分 → {decision, evidence[], policy, confidence} JSON（链外）。 */
        ...
        try {
          ...
          if (await this._shouldAudit('tool')) {
            this.auditService.log({
              ...
              // §internal.16 A-5 事件时点放行授权依据快照：对象格式（parseChecks 只认数组 → 不误判为拒绝）；
