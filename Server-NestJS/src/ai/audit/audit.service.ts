...
export class AuditService {
  constructor(
    ...
    // §internal.16 A-5 跨系统身份链：授权依据（AuthorizationExplainerService.explainAuthorization，阶段 2 切环）
  ...
  private _payload(row: object): Record<string, unknown> {
    ...
    return {
      ...
      feedback: null,
      feedbackNote: null,
      // §internal.16 A-1 business_event/evidence 是链外注解列（业务事件名 + Decision Evidence JSON，推理型展示数据）：
  ...
  private async _identityChainFromRow(
    ...
    if (!denied) {
      ...
      if (snap) {
        // §internal.17 ③ Policy Evidence：快照带策略版本时，allowed 投影携带（evidence/合规「哪一版规则允许」）；
...
/** §internal.16 A-5 放行授权快照解析：对象含 allowed:true（事件时点 checks/riskLevel/policyVersion）；拒绝数组/非放行/非法 → null */
