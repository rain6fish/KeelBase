 ...
 * - 决策：R5 阻断 / R3-R4 确认 / R0-R2 自动；策略可强制确认/放宽（§internal.15(4) mode）或禁用
 */
...
export interface ToolOverride {
  ...
  /** §internal.15(4) 门控档位覆盖：auto 放宽 / confirm·approval 强制确认（approval 在 sidecar 仍回落确认信号，双人审批在主控制面） */
...
export class SidecarToolRegistry {
   ...
   *   策略 mode/requiresConfirmation 覆盖（§internal.15(4)：auto 放宽 / confirm·approval 强制确认）
