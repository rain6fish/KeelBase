...
export class OperationAuditService {
   ...
   * DB 级串行（internal-roadmap §internal.10 B）：事务内锁 audit_chain_lock id=1 行，跨实例串行化写链（替代进程内 _tail）。
   */
  ...
  /**
   * ① 证据根（§internal.17 ①，docs/evidence-root.spec.md）：按业务对象（target_id + path 资源子串）取 REST 写链行，
  ...
  /**
   * §internal.16 A-2 业务实体行为史：按 target_id + path 资源子串反查 REST 写操作。
