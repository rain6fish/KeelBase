...
const TEMP_DB = resolve(tmpdir(), `keelbase-governance-adoption-${process.pid}.sqlite`);
...
async function finish(code, mock, children) {
  ...
  const base = resolve(__dirname, `../docs/benchmark/governance-adoption-${timestamp}`);
  ...
  console.log(`报告：docs/benchmark/governance-adoption-${timestamp}.md`);
  process.exit(code);
}
