...
async function main(): Promise<void> {
  ...
  else if (deps.queue === 'disabled')
    warn('异步队列未启用（QUEUE_ENABLED=false）：事件提醒不会触发；需部署 Redis 并设 QUEUE_ENABLED=true');
  else warn(`异步队列 ${deps.queue ?? 'unknown'}：已启用但 Redis 不可达 → 提醒与队列任务不会执行`);
