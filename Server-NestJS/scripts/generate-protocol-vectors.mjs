...
const canonicalCases = [
  ...
  {
    id: 'array-undefined-to-null',
    name: 'canonicalJSON 数组内 undefined → null（对象值 undefined 则剔除）',
    comment: 'JSON.stringify 语义：对象值为 undefined 的键被剔除（见 undefined-null-flat），但**数组元素**为 undefined 序列化为 null。JSON 语料无法表达 undefined（序列化即变 null）；金样本由生成时含 [undefined] 的内存对象产出，语料侧输入为等价 [null]。真实行为由 TS 复现测试覆盖。',
...
const canonicalFile = {
  ...
  algorithm:
    'canonicalJSON（语言中性，§2.3）：K = 顶层键剔除 undefined 后按 UTF-16 code unit 升序；序列化时在每个对象层以 K 作白名单（对象自身键序无关、不在 K 的键丢弃）；null 保留、数组内 undefined → null；数字按 ECMAScript Number::toString 最短往返（-0→0、|x|≥1e21 或 0<|x|<1e-6 用指数形）；字符串按 JSON 必需转义。行为已冻结（审计链依赖），只描述不改。',
...
const governanceBindingFile = {
  ...
  license: 'Apache-2.0',
  note: '策略（RISK_STRATEGY 值域）→ 放行决策语义 + 授权拒绝依据词表（跨 Runtime 必须一致的可解释授权词汇）。由 runner/lib/protocol-algorithms.mjs 单源生成；derivation 为 R0..R5 逐级展开（strategy → outcome）。',
