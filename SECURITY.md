...
| N-7 | "Human confirmation" does not equal an operator **individually** reviewing every item of a bulk action — a single-run batch is authorized through one **aggregated** review, not per-item sign-off; cross-turn plan-level confirmation is not provided / 不承诺「人工确认」等于业务人员**逐项**审查了每个批量动作——单轮批量经一次**聚合**审查授权，而非逐条签核；跨轮计划级确认尚未提供 | Single-run batch confirmation **is provided** (KB-5 run aggregation): the multiple write calls in one AI reply are grouped into one run and authorized once, with a per-item diff summary + risk tier (impact preview). Cross-turn action-plan confirmation — AI proposes a plan accumulated across turns, then the whole batch is authorized — is out of scope / 单轮批量确认**已提供**（KB-5 run 聚合）：一条 AI 回复中的多个写调用聚为一个 run、一次授权，并展示逐条 diff 摘要 + 风险分层（影响预览）。跨轮行动计划确认（AI 先出计划、跨轮累积后整批授权）未做 |
...
> - N-7 → 单轮 run 聚合已实现、跨轮计划仍 Out = [run-level-approval.spec.md](docs/run-level-approval.spec.md)；token 一次性防 replay = [failure-path-corpus.spec.md](docs/failure-path-corpus.spec.md) FP-2
