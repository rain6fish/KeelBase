...
| N-12 | No full Java-native edition (KeelBase4J) / 不承诺 Java 原生全套（KeelBase4J） | Existing-system access via OpenAPI / MCP / bridge proxy; java-starter is an access-layer probe; a full Java port is demand-triggered / 存量系统接入走 OpenAPI / MCP / 代理桥；java-starter 为接入层探针；整体 Java 移植由需求信号触发 |
