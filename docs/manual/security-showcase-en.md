...
## 0.5 Trust Proof Package (one-command, seven scenarios)

**Proof**: the full Business-safe Trust chain — success / permission denied / R5 block / human confirmation / revoke / evidence root — in one command, deterministic, no LLM.
...
- Seven scenarios:
  ...
  6. **Evidence root** (KB-3, cross-chain anchoring) — export a v3 evidence root for an AI write → `verify-evidence.mjs` offline verification PASS → tampering the anchor FAILs
  7. **Existing Java system** — points to the keelbase-java-starter reference project (`verify-crm-e2e.mjs`)
- Expect: all seven scenarios PASS (17/17 assertions), report at `docs/benchmark/trust-proof-*`
