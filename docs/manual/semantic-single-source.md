...
> **规则**：触及 **tool / 治理 / 审计 / 事件** 语义的实现改动，须**同批**落 `specs/protocol`（向量/金样本）或 `schemas`（wire Schema）——即「**先落契约，再改代码**」。对抗「同一语义两处实现各自漂移」。
...
**契约真源**（任一变更即视为已落契约）：`Server-NestJS/specs/protocol/`（向量/金样本/registry）、`Server-NestJS/schemas/`（wire Schema v1）。
