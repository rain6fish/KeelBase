...
> **S-1（关键能力 2.0 嵌入广度）**：业务系统**不写任何集成代码**，只把 LLM base URL 指向 sidecar，AI 调用即自动上报治理台审计（AI 流量可见性）。语言无关（任意 OpenAI 兼容 client：LangChain / LangChain4j / 自研）。
...
> **30 分钟上手**：按 [adoption-30min.md](adoption-30min.md) 5 步接入，配套 `verify-governance-adoption.mjs` 一键验收（MOAT-1）。
