...
> **定位（internal-roadmap §internal.2 生态定位收敛）**：KeelBase = AI Framework 之上的 **Enterprise AI Trust Runtime**——不重造编排。任意主流 Agent Framework（LangChain / OpenAI / Claude 等）通过 **MCP（开放标准）** 接入，即自动进入 KeelBase 治理层：Identity / Permission / Confirmation / Audit 全走通。本页演示并验证这条「MCP 即 Adapter」链路。
