...
> **定位**：M1 演示 / 评审 / 集成商选型用的一站式验证清单——每条「命令 + 前置 + 报告位置」，全部可现场复现。对应产品证明项（V-2/3/4/6）与关键能力资产（MOAT-1/2/3、S-1/S-2）。
...
## 一、关键能力：运行时治理 / Runtime Governance

| # | 验证 | 命令（仓库根） | 前置 | 报告 |
|---|---|---|---|---|
| M1 | **30 分钟接入治理**（零代码 sidecar → 治理台审计 + 哈希链） | `node Server-NestJS/scripts/verify-governance-adoption.mjs` | `npm run build`（自起治理台+sidecar+mock LLM，无需真实 key） | `Server-NestJS/docs/benchmark/governance-adoption-*`（实测 **8/8**） |
| M3 | **多系统单控制面**（一个治理台管 Node+Java 两系统） | `node Server-NestJS/scripts/demo-multi-system.mjs` | 同上（自包含） | `Server-NestJS/docs/benchmark/multisystem-*`（实测 10 条哈希链连续） |
...
```bash
npm run build   # 一次
node Server-NestJS/scripts/verify-governance-adoption.mjs     # MOAT-1：零代码接入治理
