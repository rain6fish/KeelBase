...
## 5. 国内生成式 AI 内容安全监管 / Content-safety regulation for generative AI (CN)

**定位**：国内监管要求生成式 AI 服务提供者严格履行内容安全义务、限制违规内容输出（框架：《生成式人工智能服务管理暂行办法》，2023 施行；具体要求随监管动态调整，以正式文本为准）。KeelBase 是运行时/中间件——**不替代模型层内容审核**。本节说明责任边界，以及 KeelBase 如何帮助部署方自证「AI 输出经过把关」。

### 责任边界 / Responsibility split

| 层 | 内容安全责任 |
|---|---|
| 模型层（服务提供者） | 模型内容审核、违规输出拦截——部署方应选用通过内容安全审核的模型服务 |
| 内容审核服务（部署方可选接入） | 敏感词 / 越狱 / 违规内容实时审核（第三方审核 API 或本地审核） |
| **KeelBase** | **行为治理 + 审计留痕：不产出内容、也不替代内容审核，但把审核动作纳入统一审计链，供证据追溯** |

### KeelBase 帮助项 / How KeelBase helps

| 监管关切 | KeelBase 既有能力 | 说明 |
|---|---|---|
| 「输出经过把关」可证明 | AI 审计哈希链 + 决策轨迹 | 部署方可将「内容审核调用 / 命中 / 拦截」作为审计事件落链（哈希链篡改即断链、应用边界内、可离线验证），向监管自证输出有把关且可追溯 |
| 轻量第一道内容检测 | 敏感词 / 越狱检测（见 §0 安全加固） | 输入侧轻量防线；**不替代**专业内容审核服务 |
| 高风险输出的兜底 | 人工确认（R3）/ 双人审批（R4） | 关键业务动作由人把关，收窄自动输出的风险面 |
| 责任边界清晰 | 自托管部署 + 模型 / 策略由部署方自控 | 谁提供模型谁担内容责任；KeelBase 让「企业自控 + 可审计」落地 |

> **边界小结**：内容合规义务在模型层，KeelBase 不做内容审核引擎；其价值是把「是否审、审了什么、结果如何」变成篡改即断链（应用边界内）的证据链——帮助部署方自证合规，而非代替合规。

---

## 6. 密评（密码应用安全性评估）与信创差距 / Gaps
...
| 国产 CPU/OS（麒麟/统信/arm64/龙芯） | Node 官方 arm64 构建可用；龙芯等需验证 | 盘点已实测项，只报实测（合规卡①） |
