...
## 0.5 Trust 证明包（七场景一键验证）

**证明**：Business-safe Trust 全链路——正常成功 / 越权拒绝 / R5 阻断 / 人工确认 / 撤销 / 证据根，一条命令、无 LLM 确定性跑通。
...
- 七个场景：
  ...
  6. **证据根（KB-3，跨链锚定）** — AI 写动作导出 v3 证据根 → `verify-evidence.mjs` 离线验证 PASS → 篡改锚即 FAIL
  7. **Java 存量系统** — 引导到 keelbase-java-starter 参考项目独立验证（`verify-crm-e2e.mjs`）
- 期望：7 场景全 PASS（17/17 断言），报告 `docs/benchmark/trust-proof-*`
...
- 跑：工作台 AI 对话「为辰光建材创建跟进任务」→ 弹**确认卡**（R3 写操作：风险级 + 授权技术详情 + 批准/拒绝/本轮信任）→ 批准 → 落库 →「已确认 · 可撤销」
...
- **全部可复现验证一站式清单**：[verification-index.md](verification-index.md)（治理能力 + 产品证明 + 私有化，命令/前置/报告）
