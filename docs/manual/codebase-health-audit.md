...
> 承接上一条记录里的「闸门自身的脆弱点」。四刀四个提交，均未推送；首刀起于 09-22 深夜。

- ✅ **假红收口（`772f0d04`）**：闸门原把 14 个 e2e 放在**一次** jest 调用里，撞上本机已知的「单进程长跑硬崩」
...
- ✅ **假绿收口（`678e4b99`）——排查中发现的更严重反向问题**：`Private(AIization)` 一项**恒 PASS**。
...
- ✅ **CI 与本地那 36 个 e2e 套件分片（`dbb30f84` + `64ff24d5`）**：CI 的 `test` job 与 `scripts/dev.sh`
  ...
  随后 `test:e2e:cov` 同样分片（`64ff24d5`），此举的关键在**覆盖率必须合并**：jest 每次运行都会清空
...
- ✅ **崩片重试（`a4ad1f50`）**：分片降低但不归零崩溃概率，cov 模式更重、更易中招（实测**一次四片崩两片**），
