...
> 承接上一条记录里的「闸门自身的脆弱点」。四刀，共五个代码提交（本记录为第六个），均未推送；首刀起于 09-22 深夜。**锚点记法 = hash + 标题**（本记录合入 main 时经 rebase，hash 已重写一次，故不单记 hash）。

- ✅ **假红收口（`42eb7880` release-gate e2e 分片）**：闸门原把 14 个 e2e 放在**一次** jest 调用里，撞上本机已知的「单进程长跑硬崩」
  （进程中途无输出死亡、无 jest 汇总行），缺 PASS 行 → 逐套件判定全判 FAIL；三次跑分 24/0、14/10、17/7。
  改为**轮转分 4 批**（4/4/3/3，重套件分散到不同批），汇总各批输出后**仍逐套件** grep
  `PASS test/<name>.e2e-spec.ts`（判据未削弱）；失败时**按批**回显 jest 尾部——原先只回显一次总尾部，
  会把早期套件的失败明细挤出 `tail -160` 窗口。套件清单收敛为单一真源 `E2E_SUITES`（原先 jest 调用清单与
  label 映射是两处平行列表，会漂移）。修后连跑三次均 **24/0**、退出码 0。
- ✅ **假绿收口（`aada11a2` AIization 项能失败）——排查中发现的更严重反向问题**：`Private(AIization)` 一项**恒 PASS**。
  发现方式是把环境彻底弄坏（非法 `NODE_OPTIONS`）跑闸门：**23 项如实变红，唯它仍绿**；单独跑该脚本，
  内部是「2 通过 / 3 失败」却退出码 0。四层成因缺一不可：① `verify-aiization.sh` 自计 PASS/FAIL 却
  **没有转成退出码**，而闸门只读退出码；② 其输出文件来自 `mktemp`（**会先建出空文件**），而
  `keelbase-init` 对已存在目标**默认跳过写入** → 文件恒空、三条断言无从匹配；③ `--force` 对 `--out`
  **完全无效**（`keelbase-init.mjs` 两处 `--out` 调用点漏传 `force`），帮助文案承诺的覆盖在 `--out` 上
  根本不成立；④ 断言三用全文 `grep '"name": "id"'`，而 `skipped` 里同样含该键 → 「主键被正确跳过」被读成
  「生成了 id 字段」（生成结果其实一直是对的）。修法一一对应：加 `--force` + 末尾 `[ "$FAIL" -eq 0 ]`；
  断言三改为**解析 JSON**、判 `fields` 中无 id 且非空（顺带消除空集真空真）；`writeGenerated` 改为
  **返回是否真写入**，两处 `--out` 据此传 `args.force`、跳过时打印「○ 未写入」而不再谎报 ✓；
  `--force` 帮助文案由「目标目录」放宽为「目标」。修后：单独跑 **5/0**、退出码 0；破坏环境下
  **1 通过 / 4 失败、退出码 1**；整闸门破坏跑由「23 红 + 1 假绿」变为 **0 pass / 24 fail、退出码 1**
  ——即 24 项现已**全部可红可绿**，报 `24/0` 可整体采信。
- ✅ **CI 与本地那 36 个 e2e 套件分片（`5cf27412` test:e2e 分片 + `09bfeb65` cov 分片）**：CI 的 `test` job 与 `scripts/dev.sh`
  都跑 `npm run test:e2e`，而它是 **36 套件放进一次 jest 调用**——与假红同源且暴露面比闸门那 14 个更大。
  分片做进**命令本身**（新增 `Server-NestJS/scripts/run-e2e-shards.mjs`，用 jest 原生 `--shard=N/4`）而非
  CI 的 YAML，故 CI 与本地（硬崩真正出现的地方）同源受益；分片交给 jest，**新增 e2e 文件自动纳入、
  无需维护清单**。任一片非 0 即整体非 0；失败后仍跑完四片，便于一次看到全部失败片。
  随后 `test:e2e:cov` 同样分片（`09bfeb65` cov 分片），此举的关键在**覆盖率必须合并**：jest 每次运行都会清空
  `coverageDirectory`，四片共用目录只剩最后一片——那份覆盖率看起来完整、实际只描述四分之一套件，
  比不分片更糟。各片写 `coverage-e2e/.shards/shard-N`，跑完用 istanbul（jest 传递依赖）合成
  `lcov.info` + 文本摘要，再删临时目录；片失败时仍合并（部分数据仍有参考价值），但退出码照旧反映失败。
- ✅ **崩片重试（`635f980c` 崩片重试）**：分片降低但不归零崩溃概率，cov 模式更重、更易中招（实测**一次四片崩两片**），
  故加一层对症的重试。判据收得极窄：**只在该片未产出 jest 汇总行时重试**（那意味着 jest 自己没跑完）；
  **有汇总行即真实失败，一次都不重试**——因此重试不可能掩盖失败，也**不可能掩盖 flaky**（flaky 会产出
  汇总行并判红）。每片最多 3 次尝试；每次都无汇总行则仍判失败，汇总信息里**标注为崩溃**（与测试失败区分）。
  为做此判定，runner 的输出由 `inherit` 改为**管道读取 + 实时转发 + 有界尾部缓冲**（256 KB），
  且每次尝试前清空该片自己的覆盖率目录，避免崩溃那次留下陈旧数据。
- **验证**：桩 jest 三条路径全过——某片崩一次后通过 → 整体 PASS 且**恰好重试一次**；退出码非 0 但**带汇总行**
  → **零重试**、整体 FAIL；每次都崩 → 用尽 3 次尝试后 FAIL 并标注崩溃。真实路径：`npm run test:e2e`
  **4 片首跑全过**；`npm run test:e2e:cov` 的**分片 3/4 首跑崩、重试后通过**（195 / 40 用例），
  覆盖率仍合并到 **4/4、23.86% statements**——与一次零崩溃的运行**完全相同**，证明重试未丢覆盖数据。
- ✅ **同一判据扩到 Gate1（2026-09-24 补）**：一次闸门跑里 Gate1 曾**无汇总行**失败、单独跑却 10/0——同属本机长跑
  抖动（当次系并发跑别的东西所致）。故把分片那条判据原样搬给 Gate1：**脚本没产出收尾汇总行 `Gate 1:` 才重试一次**，
  有汇总行（无论 PASS/FAIL）**不重试**；失败时回显脚本输出尾部（原先被 `>/dev/null` 吞掉）。桩验证三条路径
  （无汇总→重试后过 · 有汇总但 FAIL→零重试 · 直接过）与实测一致。
  `npm run cli:test` **143/0**；闸门回归 **24/0**。另**实测排除**两条无效解法：① 加片——coverage 的固定
