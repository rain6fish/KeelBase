...
A: 演示账号（development 首次启动自动建）：`alex / Alex@2026$Demo`（普通）、`admin / Admin@2026$KeelBase`（管理员）。管理台**必须 admin 角色**，普通用户登录管理台会 403/空白。要提权，用 admin 登录管理台 → 用户管理 → 改角色。
...
### Q: 「审计哈希链防篡改 / AI 写操作可撤销」的边界在哪？
A: 一句话：**篡改即断链、可离线验证；但只对应用边界内的写路径生效，不等于防 DBA/root，也不等于跨系统回滚**。完整边界见 [SECURITY.md → 信任边界与不承诺清单（Not-a-*）](../../SECURITY.md)：N-1/N-3 说明防篡改边界（应用边界内，不防持库/root 权限的管理员），N-4/N-5 说明「撤销」只对自身数据域 + 已声明补偿端点生效，外部系统（如 Java 侧）撤销态如实标注。
