...
> 日期 / Date: 2026-09-12
> 状态 / Status: Draft（只出规格，落地在 1.1 后；对齐 roadmap §22.18 D-2）
...
- **i18n**：`--lang zh|en`（默认 zh；报告可同时含中英，见 §7 验收）。措辞对齐 docs/manual/product-language.md。
...
- **i18n**：`--lang zh|en` 两版关键段落齐全（结论/授权/边界）。
...
## 10. 开口问题（spec 定稿前待定）/ 10. Open Questions

1. **能否直接 PDF？** 倾向：**仅 HTML + 打印样式**（浏览器「打印为 PDF」），不引 PDF 引擎（无新依赖、跨平台）。若审计场景**硬要**原生 PDF，再单评（依赖成本 ↑）。
2. **双语呈现**：`--lang` 单选 vs 报告内**中英并列**？倾向**并列**（审计现场一次交付两版，避免语言切换）。
3. **D-1 复用边界**：本报告是**单动作**；D-1（期间报告）是否**内嵌**本单动作报告（多份 HTML 合成一份含目录）还是**引用**？倾向 D-1 生成**索引 + 逐条链接**，单动作仍由本报告产出（避免重复渲染逻辑）。
