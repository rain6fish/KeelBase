...
> **状态（2026-08-30）**：以下演进方向多数已落地——协议 spec ✅、字段级审计 ✅、审计可视化 ✅、独立治理控制平面 ✅、可行动错误提示 ✅、sidecar 零代码接入 ✅（S-1）。剩余为生态/门控深化（sidecar S-2 工具门控、集成商模板）。
...
6. **治理 sidecar（零代码接入）** ✅ S-1 —— AI 网关审计代理：业务系统 LLM base URL 指向 sidecar 即自动上报治理台审计（`npm run start:sidecar`）；S-2 工具门控待续。
