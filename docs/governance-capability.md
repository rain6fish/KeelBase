...
2. **审计粒度增强** ✅ —— AI 写操作**字段级变更审计**（before/after diff，快照存副作用表独立列、**链外**）+ **行为级审计**（参数 / 上下文 / 决策依据完整入链），支撑更细粒度的合规证据。§internal.16 A-1 增 **Decision Evidence**（决策依据：decision/evidence/policy/confidence，链外列）+ **业务事件归一化**（CustomerRiskAssessed / FollowupTaskCreated 等跨系统统一业务语言）。
...
6. **治理 sidecar（零代码接入 + 工具门控）** ✅ S-1 + S-2 —— AI 网关审计代理：业务系统 LLM base URL 指向 sidecar 即自动上报治理台审计（`npm run start:sidecar`）；**工具调用按风险级门控**（R5 阻断 / R3-R4 确认 hold-and-release / R0-R2 自动，`SIDECAR_TOOLS` 配置 + 治理台策略实时覆盖），见 [governance-deploy.md](manual/governance-deploy.md)。
