...
`ref` 的 `onDelete` 映射为**数据库外键动作**，它管的是**硬删**。应用层的删除一律是**软删**（`@DeleteDateColumn`），不会触发外键动作 —— 故「**默认不级联**」在应用层始终成立，`cascade` 只在硬删（保留期清理 / 手工 SQL）时生效。另：**自引用暂不支持**（`target` 指向本模块）—— 生成时**明确报错**，不静默产出坏模块。

字段可标 **`pii: true`** 声明为个人隐私数据（**仅字符串型字段**：`string` / `text` / `enum` / `decimal`）。生成物自动获得两处接线：**管理端列表在服务端掩码**（调用方拿不到明文）、**审计 requestBody 打码**覆盖该键名（平台内建清单只认识 `email`/`phone` 等常见名，其余靠模块注册）。掩码产出字符串，故 `date` / `int` / `bool` 不可声明 —— 否则生成的代码把 string 塞进有类型的属性，编译不过。
