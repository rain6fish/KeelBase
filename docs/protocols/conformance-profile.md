...
**映射已声明（2026-09-26）**：语料只引用**对象**、不引用路径，而**各 Runtime 自己映射到自己的面**——「**由谁把这件事写下来**」已裁为**每仓一份「对象 → 端点」表**（**不进语料**）：参照实现 → `docs/wire-object-endpoints.md`；第二载体 → `KeelBase4J/docs/wire-object-endpoints.md`。**它让「本运行时没暴露该对象」与「映射写错了」在第三方那儿分得开**——两载体各自的 runner 都真跑过语料，而**表是对外可核的面**。
