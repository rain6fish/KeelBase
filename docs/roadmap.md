...
- 2026-08-12 追加「差异化定位红线」+「应用级管理（ORG）」+「获客与留存（GROWTH）」三章。来源：产品讨论——平台形态向同类产品 收敛的警觉（定位红线：不做同类产品式快速开发平台，差异主线 = 业务安全 AI Agent harness + 三端一致 + AI 引导配置）；应用端丰富功能与市面常用功能调研结论（组织架构/审批等企业能力按「应用层 + AI 织入」拟条目；社区动态流/积分等 C 端示例功能吸引试用）。
...
- 2026-08-12 补充定位红线：开发期 AI + 运行时 AI 双叙事（对照 同类产品 v3.9.2 AI Skills「一句话生成系统」）——同类产品 抢先占位「AI 生成系统」话术，但 AI 仅开发期；我们把差异从「AI 引导配置」升级为「开发期 AI（EASY-2 生成业务模块）+ 运行时 AI（HS 安全 harness）」双叙事 + 评审守则补充。来源：本次产品讨论（同类产品 观察）。
...
> 来源：2026-08-12 产品讨论——平台形态向同类产品 收敛的警觉。随着 PL/AD/POV/EASY 系列落地，管理端 CRUD、代码生成器、低代码表单、定时任务、插件、数据导入、组织架构逐步齐备——**这些恰好是同类企业脚手架 的全部地盘**。结论：不与其正面拼后台，坚守差异主线。

**一句话定位**：

> 同类产品回答「怎么快速搭一个后台界面」；KeelBase 回答「怎么快速做出一个带 AI、业务安全、三端可用的应用」。后台是送的，AI 是主线，App 是终点。

**划界（功能评审守则）**：

- **不做 / 不当卖点（同类产品地盘，hygiene）**：
  - 管理台通用 CRUD 堆料（字典、岗位、菜单管理、系统配置等纯后台 CRUD）——同类产品十年生态，正面拼必输。
...
- **评审守则**：每次功能评审先问「这功能同类产品有吗？」——若无 → 差异加分；若有 → 是否支撑差异主线，仅当支撑才做，否则标 hygiene 或不做。

**对既有待办的影响**：PL-12 组织架构从「企业版押后」重定位为「应用级管理 ORG」——按应用层 + AI 织入做，不做同类产品式部门树 CRUD 堆料；PL-13 支付维持押后（商业向）。

### 与 同类产品 的界线：开发期 AI vs 运行时 AI 双叙事（2026-08-12 补）

> 来源：2026-08-12 产品讨论——观察 同类产品 v3.9.2 推出 AI Skills「一句话生成整个系统」。它已把「AI 引导开发」做成现实并当主打卖点，**抢先占位了「AI 生成系统」这个认知**；但它生成的是同类产品式后台 CRUD，AI 是**开发期 AI**（帮你写代码，生成完就下班）。这暴露并强化了我们的差异缝隙。
...
- **结论**：同类产品 把 AI 用在「快」，我们把 AI 用在「快 + 安全 + 真的会干活」。它不构成正面威胁，但**抢占了「AI 生成系统」的话术**——我们不能再只讲「AI 引导配置」，必须讲**双叙事**：**开发期 AI**（AI 生成业务模块，EASY-2）+ **运行时 AI**（业务安全的 agent harness，HS 系列）。两者合一才完整，同类产品 只有前半段。
- **评审守则补充**：涉及「AI 生成/低代码」的功能评审，先问「这是开发期 AI 还是运行时 AI？运行时部分有没有安全链路（确认/权限/审计/撤销）？」——只有开发期 AI 的，落在同类产品地盘，标 hygiene 或升级到双叙事再做。

### 对外宣传框架：一个主线 + 三类受益方（2026-08-12 补）

> 来源：2026-08-12 产品讨论——「对外是否可称面向开发者/用户/管理者都好」。**否决弱话术「三好」**：①「都好」等于没说，且「管理台好」会被听成「后台功能全」，亲手拉回同类产品赛道；②三条平行卖点让人记不住主线——对标 Supabase/Retool/Dify 都是单角色聚焦。**替代框架**：
...
- **话术红线**：开发者那句不能只讲「快」——要讲「生成的不是后台 CRUD，而是带 AI 安全、三端可用的完整应用」，把 同类产品 落回它自己的地盘。「快」只是入场券，「安全地会干活」才是关键能力。
...
**核心澄清（用户明确）**：焦点**不是让系统自身有「快速生成功能」**（那是内建代码生成器/低代码引擎，撞同类产品地盘）；而是 **AI 能否按照系统约定快速生成相关功能**——系统提供约定（协议），AI 负责生成。系统不做生成器，AI 是生成器。
...
- 每个协议字段都要问「**AI 不生成它行不行？**」——能手写就手写，协议薄则成脚手架升级版，厚则成低代码平台（撞同类产品，且被元数据拖死可扩展性）。
...
> - 附带差异化加分：Vuetify Material 风与同类产品的 Element Plus（Ant 风）**视觉上截然不同**，「管理台不像同类产品」本身就是定位的一部分。
...
> 来源：2026-08-12 产品讨论——管理端目前只有系统级功能，区分出「应用级管理」：基座造出的应用的企业/团队能力。**调性：组织架构不是同类产品式部门树 CRUD，而是「应用层能力 + AI 织入」**——呈现用管理台，用法靠 AI 与 CASL 组织边界授权。收编 PL-12（原「多租户与组织架构」，从押后重定位）。**分期控范围：一期只做组织树 + 成员角色 + 应用级权限开关，数据隔离二期**（真正贵的是「数据按组织隔离」——现有 events/todos 全 `{userId}` 所有权，改造要动所有查询）。
...
| ORG-5 | AI 织入（差异化） | 组织边界授权的 AI 工具：查部门日程/团队空闲/审批待办统计；AI 只在组织数据范围内运行 + AI 审计带组织维度——同类产品式组织架构没有的用法 | ORG-1~4 / HS-2 | 待办 |
...
| EASY-2 | AI 生成业务模块（`npx keelbase init`，开发期 AI） | 对话式引导（做什么应用？要 AI/支付/多租户吗？目标规模？）→ 生成推荐配置集 + 种子数据 + 品牌替换 + **生成一个完整业务模块**（实体/DTO/CRUD/前端页面/权限，走基座脚手架 tool/generate_feature.sh）+ 打印「下一步」。**2026-08-12 升级：从「配置向导」升级为「AI 生成业务模块」**——同类产品 已抢先占位「一句话生成系统」，我们不能再只讲配置；配合运行时 AI（HS）构成「开发期 + 运行时」双叙事（见定位红线）。第一个 showcase 用 GROWTH-2 社区动态流（把上传/图片/通知/审计全链路演示给评估者） | AI 编排 + UX-3 脚手架已就绪 | 待办 |
