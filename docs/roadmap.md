...
- 2026-08-12 追加「差异化定位红线」+「应用级管理（ORG）」+「获客与留存（GROWTH）」三章。来源：产品讨论——平台形态向同类/Platform B 收敛的警觉（定位红线：不做同类脚手架式快速开发平台，差异主线 = 业务安全 AI Agent harness + 三端一致 + AI 引导配置）；应用端丰富功能与市面常用功能调研结论（组织架构/审批等企业能力按「应用层 + AI 织入」拟条目；社区动态流/积分等 C 端示例功能吸引试用）。
...
- 2026-08-12 补充定位红线：开发期 AI + 运行时 AI 双叙事（对照 Platform B v3.9.2 AI Skills「一句话生成系统」）——Platform B 抢先占位「AI 生成系统」话术，但 AI 仅开发期；我们把差异从「AI 引导配置」升级为「开发期 AI（EASY-2 生成业务模块）+ 运行时 AI（HS 安全 harness）」双叙事 + 评审守则补充。来源：本次产品讨论（Platform B 观察）。
...
KeelBase 的差异化 = **AI 原生 + 三端基座 + 数据主权（私有化）**：
...
| 维度 | KeelBase | BaaS（Supabase/Firebase） | AI 平台（Dify/FastGPT/Coze） | 低代码平台 |
|---|---|---|---|---|
...
> 来源：2026-08-12 产品讨论——平台形态向同类/Platform B 收敛的警觉。随着 PL/AD/POV/EASY 系列落地，管理端 CRUD、代码生成器、低代码表单、定时任务、插件、数据导入、组织架构逐步齐备——**这些恰好是同类（Platform A）与 Platform B 的全部地盘**。结论：不与其正面拼后台，坚守差异主线。

**一句话定位**：

> 同类回答「怎么快速搭一个后台界面」；KeelBase 回答「怎么快速做出一个带 AI、业务安全、三端可用的应用」。后台是送的，AI 是主线，App 是终点。

**划界（功能评审守则）**：

- **不做 / 不当卖点（同类地盘，hygiene）**：
  - 管理台通用 CRUD 堆料（字典、岗位、菜单管理、系统配置等纯后台 CRUD）——同类十年生态，正面拼必输。
  - 代码生成器（UX-3）、低代码表单（PL-10）、插件机制（PL-11）、定时任务（PL-7）当**卖点**讲——它们只是「基座完整度」的台面，不是故事主线。
- **差异主线（同类/Platform B 没有，持续加倍投入）**：
...
- **评审守则**：每次功能评审先问「这功能同类有吗？」——若无 → 差异加分；若有 → 是否支撑差异主线，仅当支撑才做，否则标 hygiene 或不做。

**对既有待办的影响**：PL-12 组织架构从「企业版押后」重定位为「应用级管理 ORG」——按应用层 + AI 织入做，不做同类脚手架式部门树 CRUD 堆料；PL-13 支付维持押后（商业向）。

### 与 Platform B 的界线：开发期 AI vs 运行时 AI 双叙事（2026-08-12 补）

> 来源：2026-08-12 产品讨论——观察 Platform B v3.9.2 推出 AI Skills「一句话生成整个系统」。它已把「AI 引导开发」做成现实并当主打卖点，**抢先占位了「AI 生成系统」这个认知**；但它生成的是同类脚手架式后台 CRUD，AI 是**开发期 AI**（帮你写代码，生成完就下班）。这暴露并强化了我们的差异缝隙。

| | Platform B 的 AI | KeelBase |
|---|---|---|
...
- **结论**：Platform B 把 AI 用在「快」，我们把 AI 用在「快 + 安全 + 真的会干活」。它不构成正面威胁，但**抢占了「AI 生成系统」的话术**——我们不能再只讲「AI 引导配置」，必须讲**双叙事**：**开发期 AI**（AI 生成业务模块，EASY-2）+ **运行时 AI**（业务安全的 agent harness，HS 系列）。两者合一才完整，Platform B 只有前半段。
- **评审守则补充**：涉及「AI 生成/低代码」的功能评审，先问「这是开发期 AI 还是运行时 AI？运行时部分有没有安全链路（确认/权限/审计/撤销）？」——只有开发期 AI 的，落在同类地盘，标 hygiene 或升级到双叙事再做。

### 对外宣传框架：一个主线 + 三类受益方（2026-08-12 补）

> 来源：2026-08-12 产品讨论——「对外是否可称面向开发者/用户/管理者都好」。**否决弱话术「三好」**：①「都好」等于没说，且「管理台好」会被听成「后台功能全」，亲手拉回同类赛道；②三条平行卖点让人记不住主线——对标 Supabase/Retool/Dify 都是单角色聚焦。**替代框架**：

**主线：业务安全的 AI Agent harness。** 三类角色不是三条平行卖点，而是主线的三个验证角度——对外话术统一导回同一主线，不同问题答对应列：

| 角色 | 一句话 | 锚定能力 | 对应问题 |
|------|--------|---------|---------|
| **开发者** | 30 分钟出一个带 AI 的安全应用 | 开发期 AI（EASY-2 生成业务模块 + EASY-6 AI 规则层） | 上手快吗？ |
| **终端用户** | AI 真的会干活，且只在你的数据范围内 | 运行时 AI（对话/工具/记忆/主动服务） | 用户真用吗？ |
| **企业主/管理** | 数据不出域、AI 每步可审计、副作用可撤销 | 治理内建（HS + POV + WEB-ADMIN 审计） | AI 安全吗？ |

- **话术红线**：开发者那句不能只讲「快」——要讲「生成的不是后台 CRUD，而是带 AI 安全、三端可用的完整应用」，把 Platform B 落回它自己的地盘。「快」只是入场券，「安全地会干活」才是关键能力。
- **已落地**：README 定位段按此改写（顶部三受益方表 + Why KeelBase 双叙事 + Business-Safe Agent Harness 段）；docs/project.spec.md 1.1 同步。
- **评审守则关联**：宣传框架的「管理者」列对应 HS 治理内建，不对外讲「管理台功能全」——管理端是 hygiene 非卖点。

### 未来方向：AI 驱动的企业应用工程体系（2026-08-13 确认）

> 来源：2026-08-13 战略讨论——用户确立发展方向：**「AI 驱动的企业应用工程体系」**。不做「大而全」平台，做「精而深」基座；通过 **协议化配置 + AI 全链路生成 + 插件化架构**，为技术团队提供「既能 10 分钟极速交付标准 CRUD，又能像搭积木一样灵活扩展复杂业务」的现代化底座。**不改变差异化红线，是红线的工程化延伸。**

**核心澄清（用户明确）**：焦点**不是让系统自身有「快速生成功能」**（那是内建代码生成器/低代码引擎，撞同类地盘）；而是 **AI 能否按照系统约定快速生成相关功能**——系统提供约定（协议），AI 负责生成。系统不做生成器，AI 是生成器。

**三支柱现状盘点**：

| 支柱 | 现状 | 成熟度 | 结论 |
|------|------|--------|------|
| 插件化架构 | PL-11 已落地（manifest + 生命周期钩子 + registerRoute + hello 示例）；缺安装/卸载 CLI + MOD-4 capabilities 联动 | ★★★☆ | 最扎实，收尾即可 |
| 协议化配置 | 零散：Settings（key-value）+ modules-manifest（依赖图）+ form-schema（表单）；**无统一业务协议层** | ★★☆☆ | **最大缺口，新方向的地基** |
| AI 全链路生成 | UX-3 generate_feature.sh 是静态脚本；EASY-2 规划中 | ★☆☆☆ | 未实现真正的 AI 生成 |

**逻辑闭环**：协议（一份 AI 可读 schema）→ AI 读协议 → 生成实体/API/页面/权限/测试/迁移 → 插件按同一协议扩展。协议是 AI 全链路生成的**输入**，不是运行时元数据。

**红线（防过度设计）**：
- 协议只覆盖**高频重复的 20%**（标准 CRUD 的 entity/DTO/API/page/permission/audit），复杂业务仍手写 + AI 辅助。
- 每个协议字段都要问「**AI 不生成它行不行？**」——能手写就手写，协议薄则成脚手架升级版，厚则成低代码平台（撞同类/Platform B，且被元数据拖死可扩展性）。
- 生成物必须 **AI 可继续扩展**（EASY-6）：生成完不是死代码，后续增量改动仍沿协议与约定走。

**优先级**：①P0 协议化配置层（新条目，最大杠杆）→ ②P1 AI 全链路生成器（EASY-2 升级）→ ③P1 插件化收尾（PL-11 CLI + MOD-4）。**HS 运行时安全不因新方向放松**——并行推进，关键能力不动摇。
...
> - 附带差异化加分：Vuetify Material 风与同类平台的 Element Plus（Ant 风）**视觉上截然不同**，「管理台不像同类」本身就是定位的一部分。
...
> 来源：2026-08-12 产品讨论——管理端目前只有系统级功能，区分出「应用级管理」：基座造出的应用的企业/团队能力。**调性：组织架构不是同类脚手架式部门树 CRUD，而是「应用层能力 + AI 织入」**——呈现用管理台，用法靠 AI 与 CASL 组织边界授权。收编 PL-12（原「多租户与组织架构」，从押后重定位）。**分期控范围：一期只做组织树 + 成员角色 + 应用级权限开关，数据隔离二期**（真正贵的是「数据按组织隔离」——现有 events/todos 全 `{userId}` 所有权，改造要动所有查询）。
...
| ORG-5 | AI 织入（差异化） | 组织边界授权的 AI 工具：查部门日程/团队空闲/审批待办统计；AI 只在组织数据范围内运行 + AI 审计带组织维度——同类脚手架式组织架构没有的用法 | ORG-1~4 / HS-2 | 待办 |
...
>
> **定位修正（2026-08-10，MIT 路线）**：**不做商业版**。壁垒 = **易用性 + 完整性 + 开箱即用**——让 KeelBase 成为"想搭私有化 AI 应用时第一个想到的起点"。不去追不在价值链上的方向（信创芯片/国产数据库适配）。新方向按 DX-1~3 优先：①本地一键跑起来（docker compose）②零基础教程 ③Taro 功能对齐。
...
| EASY-2 | AI 生成业务模块（`npx keelbase init`，开发期 AI） | 对话式引导（做什么应用？要 AI/支付/多租户吗？目标规模？）→ 生成推荐配置集 + 种子数据 + 品牌替换 + **生成一个完整业务模块**（实体/DTO/CRUD/前端页面/权限，走基座脚手架 tool/generate_feature.sh）+ 打印「下一步」。**2026-08-12 升级：从「配置向导」升级为「AI 生成业务模块」**——Platform B 已抢先占位「一句话生成系统」，我们不能再只讲配置；配合运行时 AI（HS）构成「开发期 + 运行时」双叙事（见定位红线）。第一个 showcase 用 GROWTH-2 社区动态流（把上传/图片/通知/审计全链路演示给评估者） | AI 编排 + UX-3 脚手架已就绪 | 待办 |
...
| EASY-7 | 协议化配置层（P0，新方向地基） | **统一业务协议**：把标准 CRUD 的 entity/DTO/API/page/permission/audit 描述为一份 AI 可读 schema（复用 modules-manifest 目录 + form-schema 的 JSON 形态），供 AI 全链路生成读取。**核心原则（用户 2026-08-13 明确）：焦点不是让系统内建生成器，而是 AI 能否按系统约定快速生成**——系统提供约定（协议），AI 负责生成。**红线**：协议只覆盖高频重复的 20%，每个字段问「AI 不生成它行不行？」，能手写就手写，复杂业务走手写 + AI 辅助；协议薄则成脚手架升级版，厚则成低代码平台（撞同类/Platform B）。生成物必须 AI 可继续扩展（衔接 EASY-6） | modules-manifest / form-schema / Swagger 已就绪 | 待办 |
