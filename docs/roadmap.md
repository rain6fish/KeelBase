...
| # | 条目 | 说明 | 依赖 | 状态 |
|---|------|------|------|------|
| POV-1 | 私有化 AI 部署 | 定位核心矛盾：AI 全走云端 API，与「数据主权」冲突。补：本地 LLM 集成（Ollama/vLLM，provider 加 local 类型或 base URL 指引）、本地 embedding（bge-m3 等——embedding 是难点）、云→本地降级链、私有化部署指南。**区别于 PM-8 信创芯片适配（国产硬件兼容，已移除）**——这是「数据不出域」承诺本身 | AI provider（OpenAI 兼容）已就绪 | 待办 |
...
| # | 条目 | 说明 | 依赖 | 状态 |
|---|------|------|------|------|
| PM-1 | 在线 Demo 体验站 | 报告最大可交付短板："评估只能 clone 跑，门槛高"。用 Taro H5 静态产物（`build:h5`）+ 后端 + 种子数据部署只读体验站（演示账号已具备），README 首屏放链接 | D.7 / PM-2 | 待办 |
| PM-2 | 种子演示数据（seed:demo） | `common/seed.ts` 现仅建演示账号，登录后空界面无价值感；补事件/待办/知识库/对话历史/通知的丰富样例数据，直观展示全栈基座 | 无 | 待办 |
| PM-3 | 案例墙与社区 | 无公开落地案例/标杆客户/社区分享——"看起来好≠能落地"。README 与 GitHub Discussions 征集早期使用者 PoC，哪怕小团队案例也展示 | 无 | 待办 |
