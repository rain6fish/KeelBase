...
|---|---|---|---|---|
| **P1 治理叠加** (wk1) | 每一笔 agent AI 调用可见、可审计、可算账 | 客户侧：agent LLM `base_url` → sidecar（1 行配置）；KeelBase：起 sidecar + 治理台 | `scripts/verify-governance-adoption.mjs` 8/8 通过；治理台出现 `source=sidecar` 审计（含摘要/tokens/耗时）；哈希链 `/audit/verify` 通过 | 客户 1h / KB 0.5d |
...
- 30 分钟治理接入：`scripts/verify-governance-adoption.mjs`；多系统单控制面：`scripts/demo-multi-system.mjs`

**Success metrics 验收指标（建议与客户对齐）**
| Metric | Target |
|---|---|
| AI 调用审计覆盖率 | 100%（本方案治理链路内逐笔可查，含摘要/模型/tokens/耗时） |
| 写操作确认+审计+可撤销 | 100%（治理链路内）；撤销演练 ≥1 次通过、幂等 |
| 越权/跨用户读 | 0（授权被拒且留痕） |
