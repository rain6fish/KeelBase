...
- **token 记账口径**：`chat` 行的 `promptTokens`/`completionTokens` 为**整轮对话**开销——一次用户提问若触发意图分类、上下文压缩、任务分解、子代理多轮工具循环、计划、汇总、反思等多次 LLM 调用，各笔用量累加计入同一行（唯一累加规则 = `src/ai/llm-usage.ts` 的 `addLlmUsage`）。流式（`/ai/chat/stream`）、非流式（`/ai/chat`）与知识库问答（`knowledge` 行）同口径。流式请求显式下发 `stream_options.include_usage`，否则 OpenAI 兼容供应商不回传 usage，成本统计会静默偏低；用量只在服务端记账，**不进 SSE 帧**（`done` 事件仍只有 `conversationId`，wire 契约不变）
  **Token accounting**: the `chat` row's `promptTokens`/`completionTokens` cover the **whole turn** — when one user message triggers intent classification, context compaction, task decomposition, sub-agent tool rounds, planning, summarisation or reflection calls, every call's usage is summed into that single row (the one accumulation rule lives in `src/ai/llm-usage.ts`). Streaming (`/ai/chat/stream`), non-streaming (`/ai/chat`) and knowledge-base answers (the `knowledge` row) share this accounting. A tool that calls the LLM **inside itself** (e.g. `summarize_customer_360`) reports its usage on **its own `tool_call` row** instead — tool cost stays with the tool, and that usage never enters the LLM context. Streaming requests explicitly send `stream_options.include_usage`; without it OpenAI-compatible vendors omit usage and cost statistics silently under-report. Usage is recorded server-side only and **never enters an SSE frame** (`done` still carries only `conversationId`, so the wire contract is unchanged)
