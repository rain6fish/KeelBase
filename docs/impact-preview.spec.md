...
- **不新增档位映射**：注册表 `register()` 已强制"确认写不得静默推成 none"（否则建工具即抛），故**同一时刻**此处拿到的档位与事后撤销 / 工具治理页**同源**；**跨时刻**则**以行上的快照为准** —— 撤销读 `effect.revokeClass`（`_classOf`，`src/ai/tool-effects/ai-tool-effects.service.ts`），它记的是该行**登记时**的档位，而 `ProxyTool.revokeClass` 派生自 Settings（`ai_proxy_tools` 的 `revokePath`，`src/ai/proxy/proxy-tool.ts`），运维改动即漂移。
- **未注册名容错（外部 `mcp_*` / LLM 幻觉名）**：真实注册表对未注册名抛 `Tool "x" not found`——同段 `_assertToolAllowed` / `isProxyTool` 因此都包 try/catch，本处同办：解析不到 → 返回 undefined，调用方**省略**该字段（与 `_writeImpact` 解析不到对象类型时同一诚实口径），既不抛错也不补默认"可撤销"。
  - **可达性（2026-09-17 实测 + 当日复核）**：写入时**当前到不了**（逐条路径在确认前先调 `_requiresApproval`，它当时对未注册名无守卫、先抛，工具以「执行失败」收尾）；**但同日晚些时候该前提已变**——外部工具修复（`8d6da13f`：`_riskLevelFor` 单点容错 + `_requiresApproval` 改用它）之后，**外部写工具会真正走到确认卡**，本容错因此**成为必经之路**：`_revokeClass` 对 `mcp_*` 必须**省略**而非抛错（否则外部写工具一进确认卡就被打挂）。→ **该守卫是必要的、且有回归覆盖**（`ai.service.spec` 的「外部写工具 → 走到确认卡」用例断言 `revokeClass` 被省略）。
  - **去留决定（2026-09-17，经用户确认后保留）**：保留。理由：① 失败后果不对称（未捕获异常打断整条 SSE 确认流，容错只是少显示一行）；② 与同段两处既有约定一致（`_assertToolAllowed` / `isProxyTool` 同样包 try/catch，§14.3 匹配既有风格）；③ 上述「已成必经之路」使其不再是「不可达防御」（§15.8 的顾虑不成立）。**不再作为待定项**。
...
| 只看不动（v1.1） | 撤销口径**不影响门控**：风险级 / R4 审批档 / R5 阻断 / HS-6 本轮信任一律不变——它是给审批人看的信息，不是放行依据（对齐 N-8：风险级 ≠ 业务正确性） |
...
- ~~**SSE 信封 ref 停在 v1（既有，非本切片引入）**~~ **已修（2026-09-16）**：v1 信封的 `confirmation` 属性 `$ref` 裸名 → 解析到**冻结的 v1 载荷**（`additionalProperties:false`、无 `impact` / `revokeClass`），于是本切片产出的确认帧按信封校验会**失败**——属「信封与内层对象版本脱节」。现 `sse-event` **升 v2**（`specs/protocol/schemas/v2/sse-event.schema.json`，**裸 `$id`** 以便沿用 v1 的跨文件相对 `$ref`）：**事件名枚举不变**，只把嵌套载荷 ref 跟到各对象当前版——`confirmation_request` → `confirmation-request` v3、`confirmation_decision` → `confirmation-decision` v2（后者同属旧 ref，一并收）。registry 的 `sse-event` → v2 + 两份 v2 样例（单条带顶层 `revokeClass`、run 带逐条 `revokeClass`）。**证据**：同一份 v2 样例在 v1 信封下 FAIL、在 v2 信封下 PASS；直接以内层载荷对 ref 校验，v1 报多余键 `impact` / `revokeClass`，v3 收。
  - ~~仍留一处**未改的绑定**~~ **已修（2026-09-17）**：`src/realtime/realtime.types.spec.ts` 的 `schemaOf` 原先硬编码读 `schemas/v1/`——本 v2 不改事件名枚举，故该断言当时在 v1 上仍成立、无误报；但一旦增删事件名就会变成「更新了 v2、测试仍读 v1」的**假红陷阱**。现改为**随 registry 解析对象当前版本**（同 `ai.service.spec` 的 `wiredSchema` 口径）。**双向变异验证**：把事件 const 改坏在 **v1** → 仍绿（证明确实不再读 v1）；改坏在 **v2** → 变红（证明确实读的是当前版）；还原即绿。
