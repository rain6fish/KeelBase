...
- **SSE 信封 ref 停在 v1（既有，非本切片引入）**：`specs/protocol/schemas/v1/sse-event.schema.json` 的 `confirmation` 属性仍 `$ref` 裸名 → 解析到冻结的 v1（`additionalProperties:false`，无 `impact` / `revokeClass`）。当前**无门禁**用该 ref 校验实际发射帧（`wire-schema.spec.ts` 只校验 registry 登记样例，`realtime.types.spec.ts` 只对事件名集合），故属文档级不一致而非活缺陷；若将来给 SSE 帧加校验，须先把该 ref 升到当前版本。
