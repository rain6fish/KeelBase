...
> 1.0 后第六个补丁：AI Action Center「我的 AI 行为」业务用户 Trust 面 + §22.17 ① 证据根 v3 / G-1~G-3 / P-③ 策略历史回放 + Policy Evidence（授权快照携带策略版本）+ 治理策略可视化编辑（档位化/R4 审批）——证据线「一条 AI 行为完整、不可抵赖地解释与复现」闭环；跨入口决策一致性（T5：MCP deny 审计补齐 + source=bridge + release-gate 登记）；NC-2 可执行 DX 错误前后端闭环（reason/impact/nextStep + LLM_UNAVAILABLE + 错误卡）；副作用状态服务身份反向查询（B 路径对账）。
