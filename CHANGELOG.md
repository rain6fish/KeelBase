...
- **Policy Evidence — decision-time policy version in authorization snapshots (§22.17 ③)** — `AuthorizationExplainerService.getAuthorizationReasons` now reads the governance policy once and returns `policyVersion` (= `policy.updatedAt`, null when no policy row) alongside the decision-input checks; the granted `tool_call` authorization snapshot embeds it (frozen "which rule version allowed this"); audit identity chain / compliance evidence surface it via `allowed.policyVersion` when present (legacy rows unchanged, backward compatible); denied path untouched (array shape preserved for A-8); spec `docs/audit-authz-snapshot.spec.md` §5; explainer unit spec + audit chain test green
  **Policy Evidence（§22.17 ③ 决策时策略版本冻结）**：explainer 单次取策略，`getAuthorizationReasons` 返回带 `policyVersion`（=policy.updatedAt；无策略行 null）+ 同源决策 checks；放行 tool_call 授权快照落库嵌入「哪一版规则允许」；审计身份链/合规证据 `allowed.policyVersion` 透出（历史行不变、向后兼容）；拒绝路径不变（保数组形态供 A-8）；spec docs/audit-authz-snapshot.spec.md §5 + 新 explainer 单测 + audit 链测试全绿
